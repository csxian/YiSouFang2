package com.example.jon.yisoufang2.db;
...
public class RegionDAO {
	...
	public static int getParentIdByName(String name){
		SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DBManager.DB_PATH + "/" + DBManager.DB_NAME, null);
		Cursor cursor = db.rawQuery("select * from REGIONS where name='"+name+"'",null);
		int id = -1;
		while(cursor.moveToNext()){
	...
	public static List<String> getCitybyParent(int parent){
		SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DBManager.DB_PATH + "/" + DBManager.DB_NAME, null);
